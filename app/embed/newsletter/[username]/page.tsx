...
export default function NewsletterEmbed() {
  ...
  const nlStyle = data?.user?.newsletterStyle || {};
  const styled = !!nlStyle.bgColor; // user provided their own card background

  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 12, background: "transparent" }}>
      <div style={{
        background: styled ? "transparent" : "#fff",
        borderRadius: 14,
        boxShadow: styled ? "none" : "0 4px 20px rgba(0,0,0,0.08)",
        padding: styled ? 0 : "10px 18px",
        width: "100%",
        maxWidth: 460,
      }}>
        {loading ? (
