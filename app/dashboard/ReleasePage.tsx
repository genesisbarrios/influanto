...
const ReleasePages = () => {
  ...
  useEffect(() => {
    if (editingPage?.selectedNftIds && Array.isArray(editingPage.selectedNftIds)) {
      setSelectedNftIds(editingPage.selectedNftIds);
    } else {
      setSelectedNftIds([]);
    }
  }, [editingPage?.selectedNftIds]);
  ...
  }, [editingPage?.id ?? editingPage?._id]);
...
const removeCustomLink = (index: number) => {
  ...
  const renderCollectiblesSection = () => (
    <div className="mb-4 p-4 bg-purple-50 rounded-md border border-purple-200">
      <div className="flex items-center justify-between mb-2">
        <h4 className="font-bold text-purple-800" style={{ fontFamily: font || "inherit" }}>🎵 Collectibles</h4>
      ...
      <p className="text-purple-600 text-sm mb-2" style={{ fontFamily: font || "inherit" }}>Sell your music direct to your fans.</p>
      {isLoadingNfts ? (
        ...
        <>
          {selectedNftIds.length > 0 && (
            <p className="text-xs text-purple-700 mb-2" style={{ fontFamily: font || "inherit" }}>
              {selectedNftIds.length} collectible{selectedNftIds.length !== 1 ? "s" : ""} selected
            </p>
          )}
          {showNftPicker && (
              ...
              {availableNfts.map((nft: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    {nft.imageUrl && <img src={nft.imageUrl} alt={nft.title} className="w-10 h-10 object-cover rounded" />}
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 truncate" style={{ fontFamily: font || "inherit" }}>{nft.title}</p>
